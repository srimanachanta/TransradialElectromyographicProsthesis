...
class TransformerModel(nn.Module):
    ...
    def forward(self, x):
        ...
        signal_list = self.signal_linear(signal_list.view(signal_list.shape[0], 12, -1)).view(signal_list.shape[0], -1, 12)

        signal_list1 = torch.cat([signal_list[:, :, [1, 7, 8]].view(signal_list.shape[0], -1), vertical_signals.view(signal_list.shape[0], -1)], dim=1)
        signal_list2 = torch.cat([signal_list[:, :, [0, 3, 4, 6]].view(signal_list.shape[0], -1), vertical_signals.view(signal_list.shape[0], -1)], dim=1)
        signal_list3 = torch.cat([signal_list[:, :, [0, 3, 4]].view(signal_list.shape[0], -1), vertical_signals.view(signal_list.shape[0], -1)], dim=1)
        signal_list4 = torch.cat([signal_list[:, :, [0, 3, 4, 5]].view(signal_list.shape[0], -1), vertical_signals.view(signal_list.shape[0], -1)], dim=1)
        signal_list5 = torch.cat([signal_list[:, :, [2, 9, 10, 11]].view(signal_list.shape[0], -1), vertical_signals.view(signal_list.shape[0], -1)], dim=1)
        ...
        return torch.concat([F.softmax(out1.view(signal_list.shape[0], -1, 3), dim=-1), F.softmax(out2.view(signal_list.shape[0], -1, 3), dim=-1),
                             F.softmax(out3.view(signal_list.shape[0], -1, 3), dim=-1), F.softmax(out4.view(signal_list.shape[0], -1, 3), dim=-1),
...
#         signal_list = self.signal_linear(signal_list.view(signal_list.shape[0], 12, -1)).view(signal_list.shape[0], -1, 12)
#
#         signal_list1 = signal_list[:, :, [1, 7, 8]].view(signal_list.shape[0], -1)
#         signal_list2 = signal_list[:, :, [0, 3, 4, 6]].view(signal_list.shape[0], -1)
#         signal_list3 = signal_list[:, :, [0, 3, 4]].view(signal_list.shape[0], -1)
#         signal_list4 = signal_list[:, :, [0, 3, 4, 5]].view(signal_list.shape[0], -1)
#         signal_list5 = signal_list[:, :, [2, 9, 10, 11]].view(signal_list.shape[0], -1)
#
#         out1 = self.Linear1(signal_list1)
#         out2 = self.Linear2(signal_list2)
#         out3 = self.Linear3(signal_list3)
