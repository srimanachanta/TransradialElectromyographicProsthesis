...
void sample_metrics_test() {
  ...
  for (int i = 0; i < num_samples; i++) {
    ...
    std::chrono::duration<double, std::milli> elapsed_time_millis =
  ...
  double sum = std::accumulate(latency_measurements.begin(),
                               latency_measurements.end(), 0.0);
  ...
  const double sd =
      sqrt(variance / static_cast<double>(latency_measurements.size()));

  const auto newEnd =
      std::remove_if(latency_measurements.begin(), latency_measurements.end(),
                     [&](const double val) {
  ...
  std::cout << "Num samples Filtered: "
            << num_samples - latency_measurements.size() << std::endl;
  std::cout << "Min: "
            << *std::min_element(latency_measurements.begin(),
                                 latency_measurements.end())
            << std::endl;
  std::cout << "Max: "
            << *std::max_element(latency_measurements.begin(),
                                 latency_measurements.end())
            << std::endl;

  sum = std::accumulate(latency_measurements.begin(),
                        latency_measurements.end(), 0.0);
