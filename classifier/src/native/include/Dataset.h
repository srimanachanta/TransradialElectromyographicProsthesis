...
class Dataset {
 public:
  explicit Dataset(const std::string& dataset_name) {
    ...
    const std::filesystem::path dataset_file_path =
        dataset_root_path / dataset_subpath / dataset_file_name;

    const torch::jit::script::Module dataset_container =
        torch::jit::load(dataset_file_path);

    this->x_data = dataset_container.attr("muscle_data_normalized").toTensor();
    this->y_data = dataset_container.attr("finger_state")
                       .toTensor()
                       .toType(c10::ScalarType::Int);
  }
