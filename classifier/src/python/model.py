...
class PositionalEncoding(nn.Module):
    def __init__(
        self, dimensions_model, dropout=0.0, max_len=12, position=torch.zeros(512)
    ):
        ...
        div_term = torch.exp(
            torch.arange(0, dimensions_model, 2)
            * (-math.log(10000.0) / dimensions_model)
        )
    ...
    def forward(self, x):
        x = x + self.pe[: x.size(1)].view(
            1, self.max_len, self.dimensions_model
...
class TransformerModel(nn.Module):
    def __init__(
        self, input_dim=1, d_model=32, heads=1, activation="relu", dropout=0.2
    ):
        ...
        self.encoder_layer1 = nn.TransformerEncoderLayer(
            d_model, heads, activation=activation, dim_feedforward=256
        )
        ...
        self.Linear1 = nn.Sequential(
            nn.Linear(self.signal_dim * 3 + 16 * int(d_model / heads), 256),
        ...
        self.Linear2 = nn.Sequential(
            nn.Linear(self.signal_dim * 4 + 16 * int(d_model / heads), 256),
    ...
    def forward(self, x):
        ...
        data_encoded = F.relu(
            self.encoder_whole_vertical(
                torch.flatten(x.view(x.shape[0], 16, 32, 12), 2, 3)
            )
        )
        ...
        signal_list = self.signal_linear(
            signal_list.view(signal_list.shape[0], 12, -1)
        ).view(signal_list.shape[0], -1, 12)

        signal_list1 = torch.cat(
            [
                signal_list[:, :, [1, 7, 8]].view(signal_list.shape[0], -1),
                vertical_signals.view(signal_list.shape[0], -1),
            ],
            dim=1,
        )
        signal_list2 = torch.cat(
            [
                signal_list[:, :, [0, 3, 4, 6]].view(signal_list.shape[0], -1),
                vertical_signals.view(signal_list.shape[0], -1),
            ],
            dim=1,
        )
        signal_list3 = torch.cat(
            [
                signal_list[:, :, [0, 3, 4]].view(signal_list.shape[0], -1),
                vertical_signals.view(signal_list.shape[0], -1),
            ],
            dim=1,
        )
        signal_list4 = torch.cat(
            [
                signal_list[:, :, [0, 3, 4, 5]].view(signal_list.shape[0], -1),
                vertical_signals.view(signal_list.shape[0], -1),
            ],
            dim=1,
        )
        signal_list5 = torch.cat(
            [
                signal_list[:, :, [2, 9, 10, 11]].view(signal_list.shape[0], -1),
                vertical_signals.view(signal_list.shape[0], -1),
            ],
            dim=1,
        )
        ...
        return torch.concat(
            [
                F.softmax(out1.view(signal_list.shape[0], -1, 3), dim=-1),
                ...
                F.softmax(out5.view(signal_list.shape[0], -1, 3), dim=-1),
            ],
            dim=1,
        )
