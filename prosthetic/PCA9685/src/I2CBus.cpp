...
I2CBus::I2CBus(const char *bus_filename, int address) {
    bus_fd = open(bus_filename, O_RDWR);

    if (bus_fd < 0) {
        throw std::system_error(errno, std::system_category(), "Unable to open I2C bus");
    }

    if (ioctl(bus_fd, I2C_SLAVE, address) < 0) {
        throw std::system_error(errno, std::system_category(), "Unable to set bus address");
    }
}

uint8_t I2CBus::ReadByte(int command) const {
...
void I2CBus::WriteByte(int command, uint8_t data) const {
